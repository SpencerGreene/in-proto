...
// Public datasets only — restricted data is encrypted in encrypted-data.ts
export const PUBLIC_DATA_SETS: DataSet[] = [
  { label: "Acme Corp", projects: PROJECTS, dimensions: INITIAL_DIMENSIONS },
];

// --- Crypto helpers for browser-side decryption ---
...
export async function decryptDataSets(password: string): Promise<DataSet[]> {
  const enc = new TextEncoder();
  const salt = b64ToBytes(ENCRYPTION_SALT);
  ...
  for (const blob of ENCRYPTED_DATASETS) {
    const iv = b64ToBytes(blob.iv);
    const ciphertext = b64ToBytes(blob.data);
